...
class Dataset:
    ...
    def get_item(self, di, ri):
        # di = depth image
        # with Image.open(os.path.join(self.root, item_name + "-depth.png")) as di:
        dpt_um = np.array(di)
        if self.dataset_name == "test":
            with Image.open(os.path.join("tmp_label.png")) as li:
                labels = np.array(li)
            meta = scio.loadmat(os.path.join("tmp_meta.mat"))
        # else:
        ...
        RTs, kp3ds, ctr3ds, cls_ids, kp_targ_ofst, ctr_targ_ofst = (
