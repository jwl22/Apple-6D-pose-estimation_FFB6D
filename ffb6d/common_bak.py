...
class Config:
    def __init__(self, ds_name='ycb', cls_type=''):
        ...
        if self.dataset_name == 'ycb':
            self.n_objects = 5 + 1  # 5 objects + background
            self.n_classes = self.n_objects
            self.use_orbfps = True
            self.kp_orbfps_dir = 'datasets/ycb/apple_kps/'
            ...
            self.ycb_root = os.path.abspath(
                os.path.join(
                    self.exp_dir, 'datasets/ycb/appleDataset'
                )
            )
            self.ycb_kps_dir = os.path.abspath(
                os.path.join(
                    self.exp_dir, 'datasets/ycb/apple_kps/'  # kps: keypoints
                )
            )
            ...
            self.ycb_sym_cls_ids = [1,2,3,4,5] # symmetric class?
        else:  # linemod
            self.n_objects = 5 + 1  # 5 object + background
            self.n_classes = self.n_objects
            self.lm_cls_lst = [
        ...
        self.intrinsic_matrix = {
            ...
            'blender': np.array([[700.,     0.,     320.],
                                 [0.,       700.,   240.],
                                 [0.,       0.,     1.]]),
            'ycb_K1': np.array([[644.219543457031, 0.        , 638.511718750000],
                                [0.      , 644.219543457031, 358.906250000000],
                                [0.      , 0.        , 1.0]], np.float32),
            'ycb_K2': np.array([[644.219543457031, 0.        , 638.511718750000],
                                [0.      , 644.219543457031, 358.906250000000],
                                [0.      , 0.        , 1.0]], np.float32)
        }
