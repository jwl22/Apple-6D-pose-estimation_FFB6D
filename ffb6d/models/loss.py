...
class CosLoss(_Loss):
    ...
    def forward(self, pred_ofsts, kp_targ_ofst, labels, normalize=True):
        ...
        bs, n_kpts, n_pts, c = pred_ofsts.size()
        pred_vec = pred_ofsts / (
            torch.linalg.norm(pred_ofsts, dim=3, keepdim=True) + self.eps
        )
        ...
        targ_vec = kp_targ_ofst / (
            torch.linalg.norm(kp_targ_ofst, dim=3, keepdim=True) + self.eps
        )
...
class PcldSmoothL1Loss(nn.Module):
    def __init__(self, eps=1e-5):
        super().__init__()
        self.eps = eps
        xmap = np.array([[j for i in range(672)] for j in range(376)])
